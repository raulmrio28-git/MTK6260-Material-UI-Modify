...
void mmi_fm_2micnr_rsp_microphone_volumet(void *inMsg)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_GET_AUDIO_PROFILE_RSP , NULL, MMI_FALSE);
...
void mmi_fm_2micnr_req_microphone_volumet()
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_GET_AUDIO_PROFILE_RSP, mmi_fm_2micnr_rsp_microphone_volumet, MMI_FALSE);
...
void mmi_fm_disable_2micnr_req_microphone_volumet()
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_GET_AUDIO_PROFILE_RSP, mmi_fm_disable_2micnr_rsp_microphone_volumet, MMI_FALSE);
...
#ifdef __COSMOS_MMI_PACKAGE__    
    SetKeyHandler(mmi_fm_stop_mtbf, KEY_BACK, KEY_EVENT_UP);
#endif
}
#endif /*  __MMI_FM_MTBF__ */
...
#ifdef __COSMOS_MMI_PACKAGE__
    SetKeyHandler(FmFMRadioLeftArrowHdlr, KEY_SEND,KEY_EVENT_DOWN);
    SetKeyHandler(FmFMRadioRightArrowHdlr, KEY_END, KEY_EVENT_DOWN);
    SetKeyHandler(FmFMRadioUpArrowHdlr, KEY_VOL_UP, KEY_EVENT_DOWN);
    SetKeyHandler(FmFMRadioDownArrowHdlr, KEY_VOL_DOWN, KEY_EVENT_DOWN);
#endif /*__COSMOS_MMI_PACKAGE__*/
...
static void mmi_fm_set_radio_test(U16 nDataItemId, U16 str_id, U8 index)
{
    ...
    ReadValue(nDataItemId, &(g_fm_fm_radio_cntx->Currfreq), DS_SHORT, &error);
    if (g_fm_fm_radio_cntx->Currfreq == 0xffff)
    {
        g_fm_fm_radio_cntx->Currfreq = 875;    /* init value */
        WriteValue(nDataItemId, &(g_fm_fm_radio_cntx->Currfreq), DS_SHORT, &error);
    }
...
/* 20260425 */
    ...
    if (!(*isReceiverTestOn))
    {
        *isReceiverTestOn = TRUE;
        ...
        Media_SetOutputVolume(255, 0);
        /* play MIDI_1 */
        srv_prof_play_tone_with_id(SRV_PROF_TONE_FM, AUD_ID_FACTORYTEST_EAR, SRV_PROF_RING_TYPE_REPEAT, NULL);
        mmi_fm_common_set_left_softkey(MMI_TRUE);
    }
    else
    {
...
/* 20260425 */      
        /* internal speaker amp over current protection */
        mmi_frm_set_protocol_event_handler(MSG_ID_MEDIA_IN_PROC_CALL_CNF, 
