...
static MMI_BOOL g_wgui_cat129_matui_has_arrows = MMI_FALSE;
static U16 g_wgui_cat129_matui_left_arrow = 0;
static U16 g_wgui_cat129_matui_right_arrow = 0;
static MMI_BOOL g_wgui_cat129_matui_has_title = MMI_FALSE;
static UI_string_type g_wgui_cat129_matui_title_str = NULL;
#endif
/* 20260424 */
/****************************************************************************
* Local Function
...
/* 20260424 */
#if defined(__MATERIAL_UI_THEME_MODIFY_THEMERES__)
static scrolling_text wgui_cat129_matui_title_scrooling_text;
static void wgui_cat129_matui_title_scrooling_timer(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    gui_handle_scrolling_text(&wgui_cat129_matui_title_scrooling_text);
}
static void wgui_cat129_matui_title_scrooling_text_draw_background(S32 x1, S32 y1, S32 x2, S32 y2)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    gdi_effect_alpha_blending_rect(act_layer, x1, y1, x2, y2, 128, 0, 0, 0);
    gdi_anim_set_draw_before_callback(wgui_cat129_animation_pre_callback);
	gui_pop_clip();
    //gdi_draw_solid_rect(x1, y1, x2, y2, GDI_COLOR_TRANSPARENT);
}
...
#endif
/* 20260424 */
#ifdef __MATERIAL_UI_THEME_MODIFY_THEMERES__
    gui_scrolling_text_stop(&wgui_cat129_matui_title_scrooling_text);
#endif
    /* 20260424 */
#if !defined(__MMI_SLIM_IMG_RES__)
	set_softkey_flags_off(UI_BUTTON_DISABLE_BACKGND_DISPLAY, MMI_LEFT_SOFTKEY);
...
/* 20260424 */
#if defined(__MATERIAL_UI_THEME_MODIFY_THEMERES__)
static void wgui_cat129_draw_matui_titlebar(dm_coordinates *coordinate)
{
    S32 sw, sh;
    S32 x, y, w, h;
    stFontAttribute title_font;
	...
	S32 x1, y1, x2, y2;
    S32 aroow_left_w = 0;
    S32 all_arrow_width = 0;
    /* titlebar bkg alpha */
    x1 = 0;
	y1 = CFG_UI_STATUS_BAR_HEIGHT;
	x2 = UI_device_width - 1;
	y2 = CFG_UI_STATUS_BAR_HEIGHT + CFG_UI_THIN_TITLE_HEIGHT - 1;
    ...
    if ((act_layer != src_layer) && src_layer)
        gdi_effect_alpha_blending_rect(src_layer, x1, y1, x2, y2, 128, 0, 0, 0);
    else if (act_layer)
        gdi_effect_alpha_blending_rect(act_layer, x1, y1, x2, y2, 255, 0, 0, 0);

    if (g_wgui_cat129_matui_has_arrows == MMI_FALSE)
    {
        gdi_layer_pop_clip();
        return;
    }
    ...
    if (wgui_is_touch_title_bar_buttons())
    {
        gui_measure_image(get_image(g_wgui_cat129_matui_left_arrow), &sw, &sh);
        x = 0;
        y = CFG_UI_STATUS_BAR_HEIGHT + ((CFG_UI_THIN_TITLE_HEIGHT - sh) >> 1);
        all_arrow_width += sw;
        aroow_left_w = sw;
        gdi_image_draw_id_blend2layers(x, y, g_wgui_cat129_matui_left_arrow);

        gui_measure_image(get_image(g_wgui_cat129_matui_right_arrow), &sw, &sh);
        x = UI_device_width - sw;
        y = CFG_UI_STATUS_BAR_HEIGHT + ((CFG_UI_THIN_TITLE_HEIGHT - sh) >> 1);
        all_arrow_width += sw;
        gdi_image_draw_id_blend2layers(x, y, g_wgui_cat129_matui_right_arrow);
    }

    if (g_wgui_cat129_matui_has_title == MMI_FALSE)
    {
        gdi_layer_pop_clip();
        return;
    }
    ...
    gui_measure_string(g_wgui_cat129_matui_title_str, &w, &h);	
    x = (UI_device_width-w) >> 1;
    y = CFG_UI_STATUS_BAR_HEIGHT + ((CFG_UI_THIN_TITLE_HEIGHT - h) >> 1);

    if(w > UI_device_width- all_arrow_width-2)
	{
	    gui_create_scrolling_text(
				&wgui_cat129_matui_title_scrooling_text,
				aroow_left_w+1,
				y,
				UI_device_width- all_arrow_width-2,
				h,
				g_wgui_cat129_matui_title_str,
				wgui_cat129_matui_title_scrooling_timer,
				wgui_cat129_matui_title_scrooling_text_draw_background,
				UI_COLOR_WHITE,
				UI_COLOR_NONE);
	    gui_show_scrolling_text(&wgui_cat129_matui_title_scrooling_text);
	}
	else
	{
        gui_set_font(&title_font);
        ...
        gui_print_text(g_wgui_cat129_matui_title_str);
    }
...
/* 20260424 */
#if defined(__MATERIAL_UI_THEME_MODIFY_THEMERES__)
    g_wgui_cat129_matui_has_title = MMI_FALSE;
    if (title != NULL)
    {
        g_wgui_cat129_matui_has_title = MMI_TRUE;
        g_wgui_cat129_matui_title_str = title;
    }
...
/* 20260424 */
#if defined(__MATERIAL_UI_THEME_MODIFY_THEMERES__)
    g_wgui_cat129_matui_has_arrows = MMI_FALSE;
    if (wgui_is_touch_title_bar_buttons())
    {
        g_wgui_cat129_matui_has_arrows = MMI_TRUE;
        g_wgui_cat129_matui_left_arrow = LEFT_RED_ARROW;
        g_wgui_cat129_matui_right_arrow = RIGHT_RED_ARROW;
    }
#else
    if (wgui_is_touch_title_bar_buttons())
    {
...
#endif
/* 20260424 */
#if !(defined(__MMI_BASIC_UI_STYLE__) || defined(__MMI_FTE_SUPPORT_SLIM__) || defined(__MMI_SLIM_IMG_RES__) || defined(__MATERIAL_UI_THEME_MODIFY_THEMERES__))
/* 20260424 */
        title_bg_id = IMAGE_VIEW_TITLEBAR_BG;   /* for showing different title bar. */
#endif
...
/* 20260424 */
    gdi_layer_unlock_frame_buffer();
/* 20260424 */
#if defined(__MMI_TOUCH_SCREEN__) && !defined(__MATERIAL_UI_THEME_MODIFY_THEMERES__)
/* 20260424 */
    if (wgui_is_touch_title_bar_buttons())
    {
        /* register the handler for pen event down istead of pen event up */
...
/* 20260424 */
#if defined(__MATERIAL_UI_THEME_MODIFY_THEMERES__)
        dm_register_category_controlled2_callback(wgui_cat129_draw_matui_titlebar);
#endif
/* 20260424 */
    }
    else
