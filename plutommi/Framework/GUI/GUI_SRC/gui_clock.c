...
/* 20260423 */
#ifdef __MATERIAL_UI_THEME_MODIFY_IDLE__
S32 g_gui_clock_matui_h = 0;
S32 g_gui_clock_matui_m = 0;
kal_bool g_gui_clock_matui_is_ampm = KAL_FALSE;
S32 g_gui_clock_matui_ampm_mode = 0;
#endif
/* 20260423 */
...
/* 20260423 */
    ...
    clock_clear_x = c->x;
#endif
    gui_set_clip(clock_clear_x, c->y, c->x + c->width -1, c->y + c->height -1);
    if (c->hide_callback != NULL)
    {
        c->hide_callback(clock_clear_x, c->y, c->x + c->width -1, c->y + c->height -1);
    }
    else
    {	
        gdi_draw_solid_rect(
...
/* 20260423 */
#ifdef __MATERIAL_UI_THEME_MODIFY_IDLE__
    g_gui_clock_matui_h = time_to_show->nHour;
    g_gui_clock_matui_m = time_to_show->nMin;
	g_gui_clock_matui_is_ampm = (PhnsetGetTimeFormat() == 0) ? KAL_TRUE : KAL_FALSE;
    if (g_gui_clock_matui_is_ampm)
		g_gui_clock_matui_ampm_mode = (hours >= 12) ? 1 : 0;
#endif
/* 20260423 */
...
/* 20260423 */
#ifdef __MATERIAL_UI_THEME_MODIFY_IDLE__
    gdi_image_get_dimension_id(
        IMG_TECHNO_DIGITAL_CLOCK_0 + num1,
        &num_width,
        &num_height);
    ...
    /* second num of hour */
    gdi_image_get_dimension_id(
        IMG_TECHNO_DIGITAL_CLOCK_0 + num2,
        &num_width,
        &num_height);
    ...
    case GUI_CLOCK_MAINLCD_TYPE_DIGILTAL_DAY_DATE_ONE_LINE:
        {
                ...
                mmi_asc_to_ucs2((S8*) string, ts);
            #else /* __MMI_DATE_TIME_IN_DIFF_LANG__ */
                day_str = get_string((MMI_ID_TYPE) (STR_GUI_TIME_DAY_SUNDAY + day_index));
                sprintf((S8*) ts, " %02d ", day);
                month_str = get_string((MMI_ID_TYPE) (STR_GUI_MONTH_BASE + month));
                if (day_str)
                {
                    UI_strcpy(string, day_str);
                }
                else
                {
                    UI_strcpy(string, L"");
...
/*20260423*/
#ifdef __MATERIAL_UI_THEME_MODIFY_IDLE__
    if (c->type == GUI_CLOCK_MAINLCD_TYPE_DIGITAL_TIME_IMAGE
        && g_dm_data.s32CatId == MMI_CATEGORY33_ID) /*time display fix*/
        clock_x = 0;
    else
		clock_x = c->x;
#else
	clock_x = c->x;
#endif
    gdi_layer_set_clip(clock_x, c->y, c->x + c->width - 1, c->y + c->height - 1);

    if (c->hide_callback != NULL)
    {
        c->hide_callback(clock_x, c->y, c->x + c->width - 1, c->y + c->height - 1);
    }
/*20260423*/
    switch (c->type)
    {
...
/*20260423*/
    ...
    {
        c->post_display_callback(clock_x, c->y, c->x + c->width - 1, c->y + c->height - 1);
    }
    gdi_layer_unlock_frame_buffer();
    gdi_layer_blt_previous(clock_x, c->y, c->x + c->width - 1, c->y + c->height - 1);
/*20260423*/
}
        ...
        *h = image_h;
        {
            ...
            S32 m_d2, m_d1;
            S32 t_w, t_h;
            t_hour = g_gui_clock_matui_h;
            if (g_gui_clock_matui_is_ampm == KAL_TRUE)
            {
                if (g_gui_clock_matui_ampm_mode == 1 && t_hour > 12)
                    t_hour -= 12;
            }
            h_d2 = t_hour % 10;
            h_d1 = (t_hour-h_d2) / 10;
            gdi_image_get_dimension_id(
                IMG_TECHNO_DIGITAL_CLOCK_0 + h_d1,
                &t_w, &t_h
            );
            *w += t_w;
            gdi_image_get_dimension_id(
                IMG_TECHNO_DIGITAL_CLOCK_0 + h_d2,
                &t_w, &t_h
            );
            *w += t_w;
            gdi_image_get_dimension_id(
            ...
            *w += t_w;
			m_d2 = g_gui_clock_matui_m % 10;
            m_d1 = (g_gui_clock_matui_m-m_d2) / 10;
            gdi_image_get_dimension_id(
                IMG_TECHNO_DIGITAL_CLOCK_0 + m_d1,
                &t_w, &t_h
            );
            *w += t_w;
            gdi_image_get_dimension_id(
                IMG_TECHNO_DIGITAL_CLOCK_0 + m_d2,
                &t_w, &t_h
            );
            *w += t_w;
            if (g_gui_clock_matui_is_ampm == KAL_TRUE)
