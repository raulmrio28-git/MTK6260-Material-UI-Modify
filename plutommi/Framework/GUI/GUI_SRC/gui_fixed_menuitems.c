...
void gui_show_animation_with_background_filler(S32 x, S32 y, PU8 animation, UI_filled_area *animation_background_filler, gdi_handle *animation_handle)
{
    ...
    anim_with_bkg_p->x = x;
    anim_with_bkg_p->y = y; 
//20260213 center highlight mainmenu
    {
        kal_int32 w = 0, h = 0;
		kal_int32 dw = 0, dh = 0;
        if (!animation_background_filler)
        {
            goto continue_image;
		}
        if (!animation_background_filler->b)
        {
            goto continue_image;
        }
		gui_measure_image(animation_background_filler->b, &dw, &dh);
        gdi_image_get_dimension(animation, &w, &h);
		anim_with_bkg_p->x += ((w-dw) >> 1);
    }
    continue_image:
//20260213 center highlight mainmenu
